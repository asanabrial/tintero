/**
 * SQLite content schema — per-dialect schema file for the content layer.
 *
 ...
 *   - Timestamps: integer (epoch milliseconds, UTC) — SQLite has no native date type
 *   - Booleans: integer 0/1 — SQLite has no native boolean type
 *
 * The logical shape is kept identical to schema.pg.ts. The conformance test
...
export const content = sqliteTable(
  "content",
  {
    ...
    // SQLite does not have a separate FK constraint type at the table-builder level;
    // FK enforcement requires PRAGMA foreign_keys = ON at connection time.
    // The column is defined identically to the pg version for logical conformance.
...
export const term_relationships = sqliteTable(
  "term_relationships",
  {
...
export const content_meta = sqliteTable(
  "content_meta",
  {
    id: text("id").primaryKey(),
    content_id: text("content_id").notNull(),
