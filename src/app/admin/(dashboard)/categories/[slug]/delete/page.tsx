// Category delete confirmation page — server component.
// NO 'use cache' — verifySession() reads cookies (forces dynamic rendering, D11).
// Plain server form action — mirrors users/[id]/delete pattern.
// No useActionState island needed: plain destructive form on confirm.
...
async function DeleteCategoryContent({ params }: DeleteCategoryContentProps) {
  await verifySession();
  ...
  const affectedCount = findAffectedPosts(
    posts,
    "categories",
    entry.label
  ).length;

  const boundAction = deleteCategoryAction.bind(null, entry.label);

  return (
    <div>
      <h1 className="text-2xl font-bold text-zinc-900 dark:text-zinc-50 mb-6">{t(loc, "admin.taxonomy.deleteCategoryTitle")}</h1>
      <p className="text-sm text-zinc-600 dark:text-zinc-400">
        {t(loc, "admin.taxonomy.aboutToDeleteCategory", { label: entry.label })}
      </p>
      ...
      <div className="rounded-md bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 px-3 py-2 mt-4">
        <p className="text-sm text-amber-700 dark:text-amber-400">
          {t(loc, "admin.taxonomy.deleteWarningCategory", { label: entry.label })}
        </p>
      </div>
...
export default function DeleteCategoryPage({
  ...
  return (
    <Suspense fallback={<p>Loading…</p>}>
      <DeleteCategoryContent params={params} />
    </Suspense>
  );
}
